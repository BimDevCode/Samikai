    ...
    RabbitMqTelemetry rabbitMqTelemetry) : IEventBus, IDisposable, IHostedService
{
    ...
    public Task PublishAsync(IntegrationEvent @event)
    {
        ...
        if (logger.IsEnabled(LogLevel.Trace)) logger.LogTrace("Creating RabbitMQ channel to publish event: {EventId} ({EventName})", @event.Id, routingKey);

        using var channel = _rabbitMqConnection?.CreateModel() ?? throw new InvalidOperationException("RabbitMQ connection is not open");

        if (logger.IsEnabled(LogLevel.Trace)) logger.LogTrace("Declaring RabbitMQ exchange to publish event: {EventId}", @event.Id);
        ...
        return policy.ExecuteAsync(() =>
        {
            ...
            else if (activity != null) contextToInject = activity.Context;
    ...
    private async Task OnMessageReceived(object sender, BasicDeliverEventArgs eventArgs)
    {
        ...
        try
        {
            ...
            if (message.Contains("throw-fake-exception", StringComparison.InvariantCultureIgnoreCase)) throw new InvalidOperationException($"Fake exception requested: \"{message}\"");
